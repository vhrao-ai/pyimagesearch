...
from pyimagesearch.traffic_sign_net import TrafficSignNetwork
...
def load_split(base_path, csv_path):
    ...
    for (i, row) in enumerate(rows):
        ...
        # resize the image to be 32x32 pixels, ignoring aspect ratio, and then perform Contrast Limited Adaptive
...
model = TrafficSignNetwork.build(width=32, height=32, depth=3, classes=numLabels)
...
H = model.fit_generator(aug.flow(trainX, trainY, batch_size=BS), validation_data=(testX, testY),
                        steps_per_epoch=trainX.shape[0] // BS, epochs=NUM_EPOCHS, class_weight=classWeight, verbose=1)
...
print("[INFO] Serializing the network to '{}'...".format(args["model"]))
