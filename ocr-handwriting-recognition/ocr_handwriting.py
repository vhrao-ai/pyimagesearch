...
ap.add_argument("-m", "--model", type=str, required=True, help="Path to the trained handwriting recognition model")
...
# Perform edge detection, find contours in the edge map and sort the resulting corners from left-to-right
...
for c in cnts:
    ...
    if (5 <= w <= 150) and (15 <= h <= 120):
        # Extract the character and threshold it to make the character appear as *white* (foreground)
        # on a *black* background, then grab the width and height of the thresholded image
        ...
        # Re-grab the image dimensions (now that its been resized) and then determine how much we need to pad the width
        # and height such that the image will be 32x32
        (tH, tW) = thresh.shape
