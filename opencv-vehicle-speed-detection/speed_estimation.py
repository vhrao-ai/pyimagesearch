...
#   USAGE
# -----------------------------
# python speed_estimation.py --conf config/config.json
...
print("[INFO] Loading the serialized model...")
...
# Instantiate the centroid tracker, then initialize a list to store each of our dlib correlation trackers,
# followed by a dictionary to map each unique object ID to a TrackableObject
ct = CentroidTracker(max_disappeared=conf["max_disappear"], max_distance=conf["max_distance"])
trackers = []
trackableObjects = {}
...
while True:
    ...
    if logFile is None:
        ...
        # if we are using dropbox and this is a empty log file then write the column headings
        if conf["use_dropbox"] and pos == 0:
            logFile.write("Year,Month,Day,Time,Speed (in MPH),ImageID\n")
        # otherwise, we are not using dropbox and this is a empty log file then write the column headings
        elif pos == 0:
    ...
    for (objectID, centroid) in objects.items():
        ...
        # not yet been estimated then estimate it
            ...
            if to.direction > 0:
                ...
                if to.timestamp["A"] == 0:
                    # if the centroid's x-coordinate is greater than the corresponding point then set the timestamp
                    # as current timestamp and set the position as the centroid's x-coordinate
                ...
                # check to see if timestamp has been noted for point D
                    ...
                    # if the centroid's x-coordinate is greater than the corresponding point then set the timestamp
                    # as current timestamp, set the position as the centroid's x-coordinate, and set the last point
                    # flag as True
                    if centroid[0] > conf["speed_estimation_zone"]["D"]:
                        to.timestamp["D"] = ts
                        to.position["D"] = centroid[0]
                        to.lastPoint = True
            # if the direction is negative (indicating the object is moving from right to left)
            elif to.direction < 0:
                # check to see if timestamp has been noted for point D
                if to.timestamp["D"] == 0:
                    # if the centroid's x-coordinate is lesser than the corresponding point then set the timestamp
                    # as current timestamp and set the position as the centroid's x-coordinate
                    if centroid[0] < conf["speed_estimation_zone"]["D"]:
                        to.timestamp["D"] = ts
                        to.position["D"] = centroid[0]
                ...
                # check to see if timestamp has been noted for point A
                    ...
                    if centroid[0] < conf["speed_estimation_zone"]["A"]:
                        ...
                        to.position["A"] = centroid[0]
                        to.lastPoint = True
            # check to see if the vehicle is past the last point and the vehicle's speed has not yet been estimated,
            # if yes, then calculate the vehicle speed and log it if it's over the limit
        ...
        if not to.logged:
            ...
            if to.estimated and to.speedMPH > conf["speed_limit"]:
                ...
                if conf["use_dropbox"]:
                    ...
                    t = Thread(target=upload_file, args=(tempFile, client, imageID,))
                    t.start()
                ...
                to.logged = True
    # if the *display* flag is set, then display the current frame to the screen and record if a user presses a key
    if conf["display"]:
