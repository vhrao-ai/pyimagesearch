...
#   FUNCTIONS
# -----------------------------
def detect_and_predict_mask(frame, faceNet, maskNet, detect_confidence):
    ...
    if len(faces) > 0:
        # For faster inference we'll make batch predictions on *all* faces at the same time rather
        # than one-by-one predictions in the above `for` loop
...
# Load the serialized face detection model from disk
